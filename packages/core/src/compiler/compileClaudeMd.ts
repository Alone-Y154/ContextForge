...
const CLAUDE_INSTRUCTIONS_DIR = ".contextforge/instructions/claude";

async function readInstructionSummaries(root: string): Promise<string[]> {
  const directory = path.join(root, CLAUDE_INSTRUCTIONS_DIR);
...
export async function compileClaudeMd(root: string, packs: InstalledPack[]): Promise<string> {
